...
A **carried-only bridge is a stateless relay** and needs no failover. It holds no authoritative state: carried content is self-verifying end to end, fencing high-water marks live at the resource, and safe states fire at the endpoints. Several such bridges therefore run side by side, all active, the multiplicity pattern of Section 9 rather than a redundancy pool. With more than one live, a single bridge death is invisible: content and heartbeats keep arriving by the others, so nothing goes stale. The destination verifies and converges regardless of path, so duplicate forwards are harmless on the control plane; on the real-time event plane the consumer removes duplicates by sequence, the ordinary cost of multipath delivery.
