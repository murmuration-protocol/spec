...
**11. Substrate adoption and the concrete identifier encoding.** Adopt an off-the-shelf substrate such as libp2p, or reimplement identity, discovery, and negotiation, and on which substrate? Much of the original fork has since settled, narrowing what stays open. The identifier construction family is fixed in Section 3 at its general, rotation-surviving shape: an inception key plus a signed key-event log, the KERI shape, with the raw-key and digest-of-key forms as its degenerate. The wire encoding is resolved to deterministic CBOR (question 33, Section 7.1), and the reference signature and hash algorithms are fixed at Ed25519 and SHA-256, algorithm-tagged for agility (Section 7.1). The libp2p branch is effectively refuted by those resolutions: libp2p uses protobuf pervasively, so adopting it would reintroduce an encoding the mandate has ruled out, and it would impose digest-of-key peer identifiers, the degenerate construction. What genuinely stays open is therefore narrower: the reimplement-versus-adopt choice and which substrate carries it, and the concrete byte encoding of the identifier that rides that substrate, not the construction family, the wire encoding, or the reference algorithms. The rotation-surviving form composes with a threshold-held current controlling key (Section 11), which removes the single key-holder and is the form question 20 wants. The witness runs the degenerate identifier and a durable principal the general one, with no new identity code between them.
