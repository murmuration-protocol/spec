...
- **Bridge**: a node that speaks two profiles and relays a contract across them (Section 6.3).
...
The core default is the **trusted single steward** (a synth rig, a single-owner vehicle subsystem): election collapses to "the steward picks", no consensus needed, a ground exists. The harder tiers, crash-fault consensus among peers with no trusted arbiter and Byzantine consensus among mutually distrusting parties, are the [federation extension](extensions/federation.md).
...
34. **The profile catalogue and the scoped interoperability floor** (Section 6.3). A profile realizes the transport match (Section 6.1) for one substrate, and the interoperability floor is per-contract, never global. Open: how small and how governed the standard catalogue must be so it does not sprawl into a private profile per vendor, which is the definition-commons governance (the updates extension) and the anti-consortium stance (Section 4.5) applied to profiles; whether a per-contract floor suffices or some deployments need a wider declared interoperability scope; and the minimum a profile must pin (encoding, discovery, per-requirement satisfaction) to guarantee blind interoperation. Interlocks with questions 11 and 33, since the substrate and encoding choices bound what the first profiles can be.
35. **Bridge semantics across substrates** (Section 6.3). A bridge relays a contract across two profiles. Carried authority, authenticity, and fencing cross unchanged; ambient authority cannot, and must be re-established as a logged re-binding. Open: whether a safety property must be guaranteed to hold *across* a bridge (an override intent originating on one substrate reaching an actuator on another), or only per leg, which is the cross-substrate face of question 12; the precise trust-transfer and fencing-re-scoping rules when a bridge re-originates authority; and how a bridge advertises which capabilities its two legs can jointly carry without weakening the end-to-end transport match. Interlocks with questions 12 and 19.
