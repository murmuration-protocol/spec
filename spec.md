...
A profile pins three things: the transport mapping that carries the wire-encoded contract messages of Section 7.1 onto the substrate, the discovery mechanism, and, for each requirement of Section 6.1, how the substrate satisfies it or that it cannot. This mapping is the substrate's addressing, its message boundaries, and which reliability channel each message uses. The wire encoding itself is universal and not a profile choice, so two profiles carry byte-identical artifacts and differ only in how they move them. A requirement the substrate cannot meet is declared unmet, never silently emulated. Capabilities that require it then cannot bind over that profile, because the match refuses otherwise (Section 6.1). This is the imperfect matrix made honest: an unavailable capability is a substrate declaring its limits, not a hidden failure.
...
The first is a **format version**: a single monotonically increasing integer, carried in every artifact, never a semantic version. It governs the canonical-CBOR rules and the envelope layout, and nothing else. It is incremented whenever those encoding rules or that shape change, so exactly one interpretation is ever in force for a given artifact, and a reader that does not recognize a version rejects the artifact rather than misreading it. The format version deliberately does not name the cryptographic algorithms, so a weakened cipher never forces a wire-shape revision and the wire shape never gates a crypto migration.

The second is **algorithm agility**, carried separately. The hash that content-addresses an artifact and the signature that authenticates it are each named by an explicit algorithm tag drawn from an open namespace, never assumed by position. The reference algorithms are SHA-256 for content addresses and Ed25519 for signatures. The tag is an opaque registry identifier, not a position on an ordered ladder, so naming a new algorithm implies no progression and no blessed suite. The specification stewards this namespace rather than dictating it, and a reserved private range lets a deployment adopt a new algorithm under its own authority before any registration. The specification is therefore never on the critical path of a zero-day: an owner under attack rotates to a new algorithm locally, and standardization follows rather than gates. A format that welds in its hash algorithm pays for it later, as the slow migration of long-lived systems off SHA-1 has shown. The tag is the cheap hedge, bought in advance.

Agility is safe only under rules the JWT and JOSE history made concrete, because an algorithm field an attacker can set is the vulnerability, not the hedge. The `alg: none` forgery has a token declare its own verification away, and the key-confusion attack has a key meant for one algorithm reinterpreted under another. The envelope here takes the anti-negotiation posture of PASETO, the token format built in reaction to those attacks, but realizes it through identity-binding rather than a version-pinned cipher suite. Four rules are normative. First, there is no unsecured form: no tag names the absence of protection, and an artifact lacking a valid signature or content digest under a recognized algorithm MUST be rejected, never accepted as unprotected. Second, the algorithm MUST be taken from the trusted anchor, never from the artifact under test. A signature is verified under the algorithm the signer's key declares (Section 3); a content address under the algorithm its own tag names; a tag found inside a fetched artifact is checked against the anchor, never obeyed in its place. Third, one key bears one algorithm and is never reinterpretable under another, which is precisely what the key-confusion attack abuses. Fourth, a verifier MUST enforce a local allowlist of accepted algorithms, so a retired algorithm is refused as the verifier's policy, and downgrade-resistance is the verifier's to enforce rather than the artifact's to offer. The algorithm tag and the format version are themselves covered by the signature and the content hash, so altering either breaks verification rather than redirecting it.

### 7.2 Definitions and updates

A definition's identity is **content-addressed and signed**: a hash plus a publisher key, not a mutable name on a server. The same logical artifact MUST therefore encode to identical bytes regardless of who or what produced it. The address is a hash of those bytes, so two producers that encoded it differently would name two different artifacts rather than one, and a canonical, deterministic encoding is a requirement, not a preference (Section 7.1). A verifier MUST reject a non-canonically encoded artifact rather than re-encode it to compare, so that exactly one byte form is ever valid and verification stays byte-identical. The hash algorithm is the one named by the content address being resolved, taken from that trusted reference and never from the fetched bytes, so a substituted artifact cannot redirect verification to a weaker hash (Section 7.1). Any compression or other transport transform applies outside this content-addressed form. The hash and the signature are computed over the canonical bytes, never over a compressed or transformed representation. Verification operates on the canonical bytes, once any such transform has been reversed. A relay or a cache MAY therefore compress or recompress an artifact freely, with no effect on its hash or signature. Such a definition verifies the same from the commons, a private registry, a peer, or a USB stick, with no origin server alive. Verifying a received definition is integrity and authorization: is this what publisher P published, and is P allowed to define this capability. A relaying peer confers zero authority on the content; transport trust is never content trust.
...
A pool presents one capability while internally orchestrating many; this is capability composition, with the pool as the degenerate case where all sub-members satisfy the same interface. Heterogeneous composition is recognized but deliberately out of model (Section 15.2).
...
These are the design forks. They keep a single frozen numbering, so a reference to a number from elsewhere always resolves, whatever the fork's state. The list is split by state into two parts. Section 15 holds the forks **still open**, kept visible deliberately, where objections are the most valuable contribution this specification can receive. Section 15.1 holds the forks **resolved**, each stating both the question and its answer, so a reference to a resolved number keeps the context of what was decided and why. A fork moves from one part to the other when it is decided; it never changes its number.
...
These forks are decided. Each keeps its frozen number and states both the question and the resolution, so a reference to it from elsewhere carries the context of what was decided and why. The normative text lives in the cited section; the entry here is the record of the fork.

**10. Naming.**

**Question:** What are the names of the system, the protocol, the reference daemon, and the reference implementation, and what is the contract owner called?

**Resolution:** Murmuration (the system), Murmur (the protocol), `murmurd` (the reference daemon binary); steward, never "consumer", for the contract owner. The reference implementation follows a per-language convention, `murmur-rs` first, then `murmur-go`, `murmur-haskell`, and so on, so that no single implementation wears the protocol's bare name (POSIX, not Linux). Each is a workspace holding a shared `murmur-core` library and the binaries `murmurd` (the endpoint daemon) and `murmur-bridge` (the cross-profile relay). The daemon binary is `murmurd`; `murmuration` is not used as a binary name.

**11. Substrate adoption and the concrete identifier encoding** (Sections 3 and 7.1).

**Question:** Adopt an off-the-shelf substrate such as libp2p, or reimplement identity, discovery, and negotiation? And under what concrete byte encoding does the self-certifying identifier travel? The original fork bundled the substrate strategy with several artifacts that have since settled separately: the identifier construction family (fixed in Section 3 at the rotation-surviving KERI shape, with raw-key and digest-of-key forms as its degenerate), the wire encoding (question 33), and the reference signature and hash algorithms (Ed25519 and SHA-256, Section 7.1).

**Resolution:** The identifier is a self-describing, algorithm-tagged byte structure that reuses the Section 7.1 tag mechanism, so a verifier always knows which algorithm to run, the tag distinguishing the rotation-surviving, digest-of-key, and raw-key forms, with the exact layout pinned by the conformance vectors. The substrate is reimplemented behind a thin port rather than adopted whole. An implementation MAY adopt off-the-shelf substrate machinery, but it MUST emit this identifier encoding and the mandated wire encoding (question 33), which is why a substrate welded to protobuf peer identifiers, such as libp2p, cannot serve: it would reintroduce an encoding the mandate has ruled out and impose the degenerate digest-of-key construction. Which concrete substrate the reference profile names is the profile catalogue's concern, not a global fork (question 34). The rotation-surviving form composes with a threshold-held current controlling key (Section 11), the form question 20 wants, and the witness runs the degenerate identifier and a durable principal the general one, with no new identity code between them.

**33. Wire encoding** (Section 7.1).
